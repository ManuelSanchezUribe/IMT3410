...
def Poisson_solver(mesh, problemdata):
    # Matrix assembling
    A, b = Matrix_assembling(mesh.Coordinates, mesh.Elements, mesh.Boundary[mesh.BC['Neumann'],:], problemdata.c, problemdata.f, problemdata.g)
    # Dirichlet condition
    DirichletNodes = np.unique(mesh.Boundary[mesh.BC['Dirichlet'],:])
